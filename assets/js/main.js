...
document.addEventListener("DOMContentLoaded", function() {
    ...
    initScratchpad();
    initLockScreen();
    initVoiceControl(); // Cérebro de Voz Ativo
...
// --- DICIONÁRIO DE LINKS (HUB DE VOZ) ---
const HUB_LINKS = {
    'sos': 'https://docs.google.com/forms/d/e/1FAIpQLSeFcVAOhDxNDj70FAaJC2-e2pfpnaUdtvtrhot5mW3qKvGUdA/viewform?usp=header',
    'inadimplência': 'https://docs.google.com/forms/d/e/1FAIpQLSeFcVAOhDxNDj70FAaJC2-e2pfpnaUdtvtrhot5mW3qKvGUdA/viewform?usp=header',
    'skills': 'https://accounts.g4educacao.com/sso/initialize-auth?redirectUrl=https%3A%2F%2Fg4educacao.com',
    'g4': 'https://accounts.g4educacao.com/sso/initialize-auth?redirectUrl=https%3A%2F%2Fg4educacao.com',
    'crm': 'https://app.taime.pro/',
    'taime': 'https://app.taime.pro/',
    'clicksign': 'https://app.clicksign.com/',
    'irc': 'https://evolucaoeducacional.com.br/',
    'sistema': 'https://evolucaoeducacional.com.br/',
    ...
    'diplomas': 'https://docs.google.com/spreadsheets/d/17nwesSlrCi2J_7dhdl2GRWwDLANRVIKiEshZXnY4Dyo/edit?gid=469585201#gid=469585201',
    'análise': 'https://docs.google.com/forms/d/e/1FAIpQLScT2qBNX6RaOoTc0yKaGZdN9QP6Q3cEC2WrcRH3alYr94vksA/viewform',
    'indicações': 'https://ecossistema-indicacoes.netlify.app/',
    'brasão': 'https://www.canva.com/design/DAGKLP3PwVw/zU5xwgWt2eJKzH2SJS8F5w/edit',
    'brinde': 'https://docs.google.com/forms/d/e/1FAIpQLScK5xw2bimbEYpS6d8sZcVlLS1ykjad3kOLiO9IVoDz_kbCqg/viewform',
    'ebook': 'https://docs.google.com/forms/d/e/1FAIpQLScK5xw2bimbEYpS6d8sZcVlLS1ykjad3kOLiO9IVoDz_kbCqg/viewform',
    'links': 'https://link-facil-ieferas.netlify.app/',
    'asaas': 'https://link-facil-ieferas.netlify.app/',
    'cronogramas': 'https://drive.google.com/drive/folders/1eU-VbT9iFJSxI5haOaXKTeHWUFIvat8v?usp=drive_link',
    'contratos': 'https://contratos-comercial-valores.netlify.app/',
    'tabela': 'https://tabelacomercialiefe.netlify.app/',
    'valores': 'https://tabelacomercialiefe.netlify.app/',
    'estorno': 'https://docs.google.com/forms/d/1WJsXTaVEPgLInSK55T_PI3ujznvFuIW9-e1w74hAUK0/viewform?edit_requested=true',
    'sugestões': 'https://docs.google.com/forms/d/e/1FAIpQLSdKypZSGWur_WspOeoC2ZfRsYXcDpn96JiK2Ojt5B5pyv_bHA/viewform',
    'baú': 'https://docs.google.com/forms/d/e/1FAIpQLSdKypZSGWur_WspOeoC2ZfRsYXcDpn96JiK2Ojt5B5pyv_bHA/viewform',
    'extinção': 'https://drive.google.com/file/d/1WKv6GD_q-qdyei0io9HlWozdDA5GWwJj/view',
    'anúncios': 'https://www.facebook.com/ads/library/?active_status=active&ad_type=all&country=BR&is_targeted_country=false&media_type=all&search_type=page&view_all_page_id=158275664644877',
    'fluxos': 'https://fluxos-contratos.netlify.app/'
};
...
function initAuthListener() {
    auth.onAuthStateChanged((user) => {
        ...
        } else { renderLoginButton(); localStorage.setItem('ODIN_USER_NAME', 'Visitante'); }
    });
}
function renderLoginButton() {
    ...
    const ua = document.getElementById('user-area'); const mua = document.getElementById('mobile-user-area');
    if(ua) ua.innerHTML = btn; if(mua) mua.innerHTML = btn;
}
...
function initVoiceControl() {
    ...
    recognition.onresult = (e) => {
        ...
        // --- CORREÇÃO FONÉTICA (LEAD -> LÍDER) ---
        cmd = cmd.replace(/líder/g, 'lead')
                 .replace(/lide/g, 'lead')
                 .replace(/lid/g, 'lead')
                 .replace(/leed/g, 'lead');
        ...
        if(icon) { icon.classList.remove('text-red-500', 'animate-pulse'); icon.classList.add('text-gray-400'); }
    };
    ...
    window.toggleVoice = () => {
        ...
        else { playSound('click'); recognition.start(); icon.classList.remove('text-gray-400'); icon.classList.add('text-red-500', 'animate-pulse'); }
    };
}
...
function executeVoiceCommand(cmd) {
    ...
    toast.className = "fixed top-24 left-1/2 -translate-x-1/2 bg-black/80 border border-neon-cyan text-white px-6 py-2 rounded-full backdrop-blur z-[100] animate-[fadeIn_0.3s] flex items-center gap-2 shadow-lg shadow-neon-cyan/20";
    toast.innerHTML = `<i class="fas fa-microphone text-neon-cyan"></i> <span class="font-bold text-sm">"${cmd}"</span>`;
    document.body.appendChild(toast); setTimeout(() => toast.remove(), 3000);

    // 1. Abrir Links do HUB
    if (cmd.startsWith('abrir')) {
        const target = cmd.replace('abrir', '').trim();
        for (const key in HUB_LINKS) {
            if (target.includes(key)) {
                window.open(HUB_LINKS[key], '_blank');
                window.speak(`Abrindo ${key}`);
                return;
            }
        }
    }

    // 2. Adicionar Lead (Com Correção Fonética)
    if (cmd.includes('novo lead') || cmd.includes('cadastrar lead')) {
        const name = cmd.replace('novo lead', '').replace('cadastrar lead', '').trim();
        if (name) {
            localStorage.setItem('ODIN_VOICE_PARAMS', JSON.stringify({ type: 'add_lead', name: name }));
            window.speak(`Cadastrando lead ${name}`);
            if (!window.location.href.includes('comercial.html')) window.location.href = pagesPath + '/comercial.html';
    ...
    // 3. Navegação
    if (cmd.includes('início')) window.location.href = basePath + '/index.html';
    if (cmd.includes('secretaria')) window.location.href = pagesPath + '/secretaria.html';
    if (cmd.includes('financeiro')) window.location.href = pagesPath + '/financeiro.html';
    if (cmd.includes('comercial')) window.location.href = pagesPath + '/comercial.html';
    if (cmd.includes('pedagógico')) window.location.href = pagesPath + '/pedagogico.html';
    if (cmd.includes('coordenação')) window.location.href = pagesPath + '/coordenacao.html';
    if (cmd.includes('ouvidoria')) window.location.href = pagesPath + '/ouvidoria.html';
    if (cmd.includes('ferramentas')) window.location.href = pagesPath + '/ferramentas.html';
    if (cmd.includes('bloquear')) lockScreen();
}

window.speak = (text) => {
    if(localStorage.getItem('ODIN_MUTE') === 'true') return;
    const u = new SpeechSynthesisUtterance(text); u.lang = 'pt-BR'; window.speechSynthesis.speak(u);
};

// --- NOTIFICATIONS (COM NOME) ---
...
function addLog(msg, type) {
    ...
    logs.unshift({id: Date.now(), user: user, msg: msg, type: type, time: new Date().toLocaleTimeString('pt-BR', {hour:'2-digit', minute:'2-digit'}), read: false});
...
function updateNotificationBadge() {
    ...
    if (badge) { if (unread > 0) { badge.classList.remove('hidden'); badge.innerText = unread > 9 ? '9+' : unread; } else { badge.classList.add('hidden'); } }
}
window.toggleNotifications = () => {
    const d = document.getElementById('notif-dropdown'); const l = document.getElementById('notif-list');
    ...
    if (!d.classList.contains('hidden')) {
        ...
        logs.forEach(g => {
            ...
            l.innerHTML += `<div class="p-3 border-b border-dark-700 hover:bg-dark-800 flex gap-3 items-start"><i class="fas ${icon} mt-1 text-xs"></i><div><div class="flex justify-between items-center w-full gap-2"><p class="text-[10px] font-bold text-neon-cyan truncate max-w-[120px]">${g.user||'Sistema'}</p><p class="text-[8px] text-gray-600 font-mono">${g.time}</p></div><p class="text-xs text-gray-300 leading-tight mt-0.5">${g.msg}</p></div></div>`; 
        });
...
// --- FEATURES GERAIS (Áudio, Lock, Scratchpad, Command) ---
...
function playSound(type) {
    ...
    if (type === 'click') { osc.type='sine'; osc.frequency.setValueAtTime(800,now); gain.gain.setValueAtTime(0.01,now); gain.gain.exponentialRampToValueAtTime(0.0001,now+0.03); osc.start(now); osc.stop(now+0.03); }
    else if (type === 'success') { osc.type='triangle'; osc.frequency.setValueAtTime(600,now); osc.frequency.linearRampToValueAtTime(1200,now+0.1); gain.gain.setValueAtTime(0.02,now); gain.gain.linearRampToValueAtTime(0,now+0.3); osc.start(now); osc.stop(now+0.3); }
}
...
function initLockScreen() { document.body.insertAdjacentHTML('beforeend', `<div id="odin-lockscreen" class="fixed inset-0 z-[9999] bg-black/95 backdrop-blur-xl flex flex-col items-center justify-center transition-transform duration-500 translate-y-[-100%]"><div class="text-center"><img src="${basePath}/assets/img/odin-logo.png" class="w-24 h-24 rounded-full border-4 border-neon-cyan shadow-[0_0_50px_rgba(0,240,255,0.5)] animate-pulse mb-6"><h2 class="text-3xl font-black text-white tracking-widest mb-2">LOCKED</h2><p class="text-neon-cyan font-mono text-sm mb-8" id="lock-clock">00:00</p><button onclick="unlockScreen()" class="bg-dark-800 border border-neon-cyan text-neon-cyan hover:bg-neon-cyan hover:text-black px-8 py-3 rounded-full font-bold tracking-widest transition shadow-lg flex items-center gap-2 mx-auto"><i class="fas fa-fingerprint"></i> DESBLOQUEAR</button></div></div>`); setInterval(() => { const el=document.getElementById('lock-clock'); if(el) el.innerText=new Date().toLocaleTimeString('pt-BR',{hour:'2-digit',minute:'2-digit'}); }, 1000); document.addEventListener('keydown', (e) => { if ((e.ctrlKey || e.metaKey) && e.key === 'l') { e.preventDefault(); lockScreen(); } }); }
window.lockScreen = () => { document.getElementById('odin-lockscreen').classList.remove('translate-y-[-100%]'); playSound('click'); };
window.unlockScreen = () => { document.getElementById('odin-lockscreen').classList.add('translate-y-[-100%]'); playSound('success'); };

function initScratchpad() { const s=localStorage.getItem('ODIN_SCRATCHPAD_CONTENT')||'', o=localStorage.getItem('ODIN_SCRATCHPAD_OPEN')==='true'; document.body.insertAdjacentHTML('beforeend', `<div id="scratchpad" class="fixed bottom-20 right-6 w-80 bg-dark-900 border border-dark-600 rounded-xl shadow-2xl z-[90] transition-transform duration-300 transform ${o?'translate-y-0':'translate-y-[150%]'}"><div class="flex justify-between items-center p-3 bg-dark-800 border-b border-dark-700 rounded-t-xl cursor-pointer hover:bg-dark-700 transition" onclick="toggleScratchpad()"><span class="text-xs font-bold text-neon-orange uppercase tracking-wider flex items-center gap-2"><i class="fas fa-sticky-note"></i> Notas</span><div class="flex gap-3"><i class="fas fa-trash text-gray-500 hover:text-red-500 text-xs" onclick="clearScratchpad(event)"></i><i class="fas fa-chevron-down text-gray-500 text-xs"></i></div></div><textarea id="scratchpad-area" class="w-full h-48 bg-dark-900 text-gray-300 p-3 text-xs font-mono outline-none resize-none placeholder-gray-700">${s}</textarea></div><button id="scratchpad-trigger" onclick="toggleScratchpad()" class="fixed bottom-6 right-6 w-12 h-12 bg-neon-orange text-dark-900 rounded-full shadow-lg shadow-neon-orange/20 flex items-center justify-center z-30 hover:scale-110 transition ${o?'hidden':'flex'}"><i class="fas fa-pen"></i></button>`); document.getElementById('scratchpad-area').addEventListener('input', (e) => localStorage.setItem('ODIN_SCRATCHPAD_CONTENT', e.target.value)); }
window.toggleScratchpad = () => { const p=document.getElementById('scratchpad'), b=document.getElementById('scratchpad-trigger'), h=p.classList.contains('translate-y-[150%]'); if(h){p.classList.remove('translate-y-[150%]');b.classList.add('hidden');localStorage.setItem('ODIN_SCRATCHPAD_OPEN','true');document.getElementById('scratchpad-area').focus();}else{p.classList.add('translate-y-[150%]');b.classList.remove('hidden');localStorage.setItem('ODIN_SCRATCHPAD_OPEN','false');} playSound('click'); };
...
function initCommandPalette() { const html = `<div id="odin-command" class="fixed inset-0 bg-black/90 backdrop-blur-sm z-[100] hidden flex items-start justify-center pt-32 animate-[fadeIn_0.1s]"><div class="bg-dark-900 border border-dark-600 rounded-xl w-full max-w-2xl shadow-2xl shadow-neon-cyan/20 overflow-hidden relative"><div class="p-4 border-b border-dark-700 flex items-center gap-3"><i class="fas fa-search text-neon-cyan text-xl"></i><input id="cmd-input" type="text" placeholder="Comando..." class="w-full bg-transparent text-white text-lg outline-none placeholder-gray-600 font-mono" autocomplete="off"><span class="text-[10px] bg-dark-800 text-gray-500 px-2 py-1 rounded border border-dark-700">ESC</span></div><div id="cmd-results" class="max-h-[400px] overflow-y-auto p-2"></div></div></div>`; document.body.insertAdjacentHTML('beforeend', html); document.addEventListener('keydown', (e) => { if ((e.ctrlKey || e.metaKey) && e.key === 'k') { e.preventDefault(); toggleCommandPalette(); } if (e.key === 'Escape') document.getElementById('odin-command').classList.add('hidden'); }); document.getElementById('cmd-input').addEventListener('input', (e) => renderCommands(e.target.value)); }
...
window.backupData=()=>{const d={crm:localStorage.getItem('ODIN_CRM_LEADS'),secretaria:localStorage.getItem('ODIN_SECRETARIA_KANBAN'),financeiro:localStorage.getItem('ODIN_DEBTS'),pedagogico:localStorage.getItem('ODIN_PEDAGOGICO_AGENDA'),scripts:localStorage.getItem('ODIN_USER_SCRIPTS'),logs:localStorage.getItem(LS_LOGS),scratch:localStorage.getItem('ODIN_SCRATCHPAD_CONTENT')}; const b=new Blob([JSON.stringify(d)],{type:"application/json"}); const a=document.createElement('a'); a.href=URL.createObjectURL(b); a.download=`ODIN_BACKUP.json`; a.click();}
window.restoreData=(i)=>{const f=i.files[0]; if(!f)return; if(!confirm("Restaurar?"))return; const r=new FileReader(); r.onload=(e)=>{try{const d=JSON.parse(e.target.result); if(d.crm)localStorage.setItem('ODIN_CRM_LEADS',d.crm); if(d.financeiro)localStorage.setItem('ODIN_DEBTS',d.financeiro); alert('Restaurado!'); location.reload();}catch(err){alert("Backup inválido.");}}; r.readAsText(f);}